    ...
    <el-form-item v-if="evaluateCondition(condition)" :prop="name" :rules="rules" :label-width="labelWidth">
        ...
        <template v-else>
            <el-checkbox-group v-model="modelData[name]" @change="valueChange">
                <el-checkbox v-for="(child, index) in children" :key="index" :disabled="disabled"
                    :true-label="child.trueLabel" :false-label="child.falseLabel" :label="child.value || child.name">{{
